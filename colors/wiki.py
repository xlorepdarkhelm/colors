"""Contains the implementation of the Wikipedia list of colors."""
__all__ = (
    'Wiki',
)

import enum

from colors import base


class Wiki(base.ColorGroup):
    """
    The Wiki color group.

    The following is a list of colors. A number of the color swatches below are
    taken from domain-specific naming schemes such as X11 or HTML4. RGB values
    are given for each swatch because such standards are defined in terms of
    the sRGB color space. It is not possible to accurately convert many of
    these swatches to CMYK values because of the differing gamuts of the two
    spaces, but the color management systems built into operating systems and
    image editing software attempt such conversions as accurately as possible.

    See Also:
        Wikipedia List of Colors
        
        * `https://https://en.wikipedia.org/wiki/List_of_colors:_A%E2%80%93F`
        * `https://https://en.wikipedia.org/wiki/List_of_colors:_G%E2%80%93M`
        * `https://https://en.wikipedia.org/wiki/List_of_colors:_N%E2%80%93Z`

    """

    AbsoluteZero       = base.RGBColor(246, 173, 198)
    Acajou             = base.RGBColor( 76,  47,  39)
    AcidGreen          = base.RGBColor(176, 191,  26)
    Aero               = base.RGBColor(124, 185, 232)
    AeroBlue           = base.RGBColor(201, 255, 229)
    AfricanViolet      = base.RGBColor(178, 132, 190)
    RoyalAirForceBlue  = base.RGBColor( 93, 138, 168)
    USAirForceBlue     = base.RGBColor(  0,  48, 143)
    AirSuperiorityBlue = base.RGBColor(114, 160, 193)
    AlabamaCrimson     = base.RGBColor(175,   0,  42)
    Alabaster          = base.RGBColor(242, 240, 230)
    AliceBlue          = base.RGBColor(240, 248, 255)
    AlienArmpit        = base.RGBColor(132, 222,   2)
    AlizarinCrimson    = base.RGBColor(227,  38,  54)
    AlloyOrange        = base.RGBColor(196,  98,  16)
    Almond             = base.RGBColor(239, 222, 205)
    Amaranth           = base.RGBColor(229,  43,  80)
    AmaranthDeepPurple = base.RGBColor(159,  43, 104)
    AmaranthPink       = base.RGBColor(241, 156, 187)
    AmaranthPurple     = base.RGBColor(171,  39,  79)
    AmaranthRed        = base.RGBColor(211,  33,  45)
    Amazon             = base.RGBColor( 59, 122,  87)
    Amazonite          = base.RGBColor(  0, 196, 176)
    Amber              = base.RGBColor(255, 191,   0)
    SAEAmber           = base.RGBColor(255, 126,   0)
    ECEAmber           = base.RGBColor(255, 126,   0)
    AmericanBlue       = base.RGBColor( 59,  59, 109)
    AmericanBrown      = base.RGBColor(128,  64,  64)
    AmericanGold       = base.RGBColor(211, 175,  55)
    AmericanGreen      = base.RGBColor( 52, 179,  52)
    AmericanOrange     = base.RGBColor(255, 139,   0)
    AmericanPink       = base.RGBColor(255, 152, 153)
    AmericanPurple     = base.RGBColor( 67,  28,  83)
    AmericanRed        = base.RGBColor(179,  33,  52)
    AmericanRose       = base.RGBColor(255,   3,  62)
    AmericanSilver     = base.RGBColor(207, 207, 207)
    AmericanViolet     = base.RGBColor( 85,  27, 140)
    AmericanYellow     = base.RGBColor(242, 180,   0)
    Amethyst           = base.RGBColor(153, 102, 204)
    AndroidGreen       = base.RGBColor(164, 198,  57)
    AntiFlashWhite     = base.RGBColor(242, 243, 244)
    AntiqueBrass       = base.RGBColor(205, 149, 117)
    AntiqueBronze      = base.RGBColor(102,  93,  30)
    AntiqueFuchsia     = base.RGBColor(145,  92, 131)
    AntiqueRuby        = base.RGBColor(132,  27,  45)
    AntiqueWhite       = base.RGBColor(250, 235, 215)
    EnglishAo          = base.RGBColor(  0, 128,   0)
    Apple              = base.RGBColor(102, 180,  71)
    AppleGreen         = base.RGBColor(141, 182,   0)
    Apricot            = base.RGBColor(251, 206, 177)
    Aqua               = base.RGBColor(  0, 255, 255)
    Aquamarine         = base.RGBColor(127, 255, 212)
    ArmyGreen          = base.RGBColor( 75,  83,  32)
    Arsenic            = base.RGBColor( 59,  68,  75)
    Artichoke          = base.RGBColor(143, 151, 121)
    ArylideYellow      = base.RGBColor(233, 214, 107)
    AshGray            = base.RGBColor(178, 190, 181)
    Asparagus          = base.RGBColor(135, 169, 107)
    AteneoBlue         = base.RGBColor(  0,  58, 108)
    AtomicTangerine    = base.RGBColor(255, 153, 102)
    Auburn             = base.RGBColor(165,  42,  42)
    Aureolin           = base.RGBColor(253, 238,   0)
    AuroMetalSaurus    = base.RGBColor(110, 127, 128)
    Avocado            = base.RGBColor( 86, 130,   3)
    Awesome            = base.RGBColor(255,  32,  82)
    Axolotl            = base.RGBColor( 99, 119,  91)
    AztecGold          = base.RGBColor(195, 153,  83)
    Azure              = base.RGBColor(  0, 127, 255)
    WebAzure           = base.RGBColor(240, 255, 255)
    AzureMist          = base.RGBColor(240, 255, 255)
    AzureishWhite      = base.RGBColor(219, 233, 244)

    BabyBlue            = base.RGBColor(137, 207, 240)
    BabyBlueEyes        = base.RGBColor(161, 202, 241)
    BabyPink            = base.RGBColor(244, 194, 194)
    BabyPowder          = base.RGBColor(254, 254, 250)
    BakerMillerPink     = base.RGBColor(255, 145, 175)
    BallBlue            = base.RGBColor( 33, 171, 205)
    BananaMania         = base.RGBColor(250, 231, 181)
    BananaYellow        = base.RGBColor(255, 225,  53)
    BangladeshGreen     = base.RGBColor(  0, 106,  78)
    BarbiePink          = base.RGBColor(224,  33, 138)
    BarnRed             = base.RGBColor(124,  10,   2)
    BatteryChargedBlue  = base.RGBColor( 29, 172, 214)
    BattleshipGrey      = base.RGBColor(132, 132, 130)
    Bazaar              = base.RGBColor(152, 119, 123)
    BeauBlue            = base.RGBColor(188, 212, 230)
    Beaver              = base.RGBColor(159, 129, 112)
    Begonia             = base.RGBColor(250, 110, 121)
    Beige               = base.RGBColor(245, 245, 220)
    BdazzledBlue        = base.RGBColor( 46,  88, 148)
    BigDipORuby         = base.RGBColor(156,  37,  66)
    BigFootFeet         = base.RGBColor(232, 142,  90)
    Bisque              = base.RGBColor(255, 228, 196)
    Bistre              = base.RGBColor( 61,  43,  31)
    BistreBrown         = base.RGBColor(150, 113,  23)
    BitterLemon         = base.RGBColor(202, 224,  13)
    BitterLime          = base.RGBColor(191, 255,   0)
    Bittersweet         = base.RGBColor(254, 111,  94)
    BittersweetShimmer  = base.RGBColor(191,  79,  81)
    Black               = base.RGBColor(  0,   0,   0)
    BlackBean           = base.RGBColor( 61,  12,   2)
    BlackChocolate      = base.RGBColor( 27,  24,  17)
    BlackCoffee         = base.RGBColor( 59,  47,  47)
    BlackCoral          = base.RGBColor( 84,  98, 111)
    BlackLeatherJacket  = base.RGBColor( 37,  53,  41)
    BlackOlive          = base.RGBColor( 59,  60,  54)
    Blackberry          = base.RGBColor(143,  89, 115)
    BlackShadows        = base.RGBColor(191, 175, 178)
    BlanchedAlmond      = base.RGBColor(255, 235, 205)
    BlastOffBronze      = base.RGBColor(165, 113, 100)
    BleuDeFrance        = base.RGBColor( 49, 140, 231)
    BlizzardBlue        = base.RGBColor(172, 229, 238)
    Blond               = base.RGBColor(250, 240, 190)
    BloodOrange         = base.RGBColor(209,   0,  28)
    BloodRed            = base.RGBColor(102,   0,   0)
    Blue                = base.RGBColor(  0,   0, 255)
    CrayolaBlue         = base.RGBColor( 31, 117, 254)
    MunsellBlue         = base.RGBColor(  0, 147, 175)
    NCSBlue             = base.RGBColor(  0, 135, 189)
    PantoneBlue         = base.RGBColor(  0,  24, 168)
    PigmentBlue         = base.RGBColor( 51,  51, 153)
    RYBBlue             = base.RGBColor(  2,  71, 254)
    BlueBell            = base.RGBColor(162, 162, 208)
    BlueBolt            = base.RGBColor(  0, 185, 251)
    BlueGray            = base.RGBColor(102, 153, 204)
    BlueGreen           = base.RGBColor( 13, 152, 186)
    BlueJeans           = base.RGBColor( 93, 173, 236)
    BlueLagoon          = base.RGBColor(172, 229, 238)
    BlueMagentaViolet   = base.RGBColor( 85,  53, 146)
    BlueSapphire        = base.RGBColor( 18,  97, 128)
    BlueViolet          = base.RGBColor(138,  43, 226)
    BlueYonder          = base.RGBColor( 80, 114, 167)
    Blueberry           = base.RGBColor( 79, 134, 247)
    Bluebonnet          = base.RGBColor( 28,  28, 240)
    Blush               = base.RGBColor(222,  93, 131)
    Bole                = base.RGBColor(121,  68,  59)
    BondiBlue           = base.RGBColor(  0, 149, 182)
    Bone                = base.RGBColor(227, 218, 201)
    BoogerBuster        = base.RGBColor(221, 226, 106)
    BostonUniversityRed = base.RGBColor(204,   0,   0)
    BottleGreen         = base.RGBColor(  0, 106,  78)
    Boysenberry         = base.RGBColor(135,  50,  96)
    BrandeisBlue        = base.RGBColor(  0, 112, 255)
    Brass               = base.RGBColor(181, 166,  66)
    BrickRed            = base.RGBColor(203,  65,  84)
    BrightCerulean      = base.RGBColor( 29, 172, 214)
    BrightGray          = base.RGBColor(235, 236, 240)
    BrightGreen         = base.RGBColor(102, 255,   0)
    BrightLavender      = base.RGBColor(191, 148, 228)
    BrightLilac         = base.RGBColor(216, 145, 239)
    BrightMaroon        = base.RGBColor(195,  33,  72)
    BrightNavyBlue      = base.RGBColor( 25, 116, 210)
    BrightPink          = base.RGBColor(255,   0, 127)
    BrightTurquoise     = base.RGBColor(  8, 232, 222)
    BrightUbe           = base.RGBColor(209, 159, 232)
    CrayolaBrightYellow = base.RGBColor(255, 170,  29)
    BrilliantAzure      = base.RGBColor( 51, 153, 255)
    BrilliantLavender   = base.RGBColor(244, 187, 255)
    BrilliantRose       = base.RGBColor(244, 187, 255)
    BrinkPink           = base.RGBColor(251,  96, 127)
    BritishRacingGreen  = base.RGBColor(  0,  66,  37)
    Bronze              = base.RGBColor(136,  84,  11)
    Bronze2             = base.RGBColor(205, 127,  50)
    MetallicBronze      = base.RGBColor(176, 141,  87)
    BronzeYellow        = base.RGBColor(115, 112,   0)
    Brown               = base.RGBColor(153,  51,   0)
    CrayolaBrown        = base.RGBColor(175,  89,  62)
    TraditionalBrown    = base.RGBColor(150,  75,   0)
    WebBrown            = base.RGBColor(165,  42,  42)
    BrownNose           = base.RGBColor(107,  68,  35)
    BrownSugar          = base.RGBColor(175, 110,  77)
    BrownChocolate      = base.RGBColor( 95,  25,  51)
    BrownCoffee         = base.RGBColor( 74,  44,  42)
    BrownYellow         = base.RGBColor(204, 153, 102)
    BrunswickGreen      = base.RGBColor(204, 153, 102)
    BubbleGum           = base.RGBColor(255, 193, 204)
    Bubbles             = base.RGBColor(231, 254, 255)
    BudGreen            = base.RGBColor(123, 182,  97)
    Buff                = base.RGBColor(240, 220, 130)
    BulgarianRose       = base.RGBColor( 72,   6,   7)
    Burgundy            = base.RGBColor(128,   0,  32)
    Burlywood           = base.RGBColor(222, 184, 135)
    BurnishedBrown      = base.RGBColor(161, 122, 116)
    BurntOrange         = base.RGBColor(204,  85,   0)
    BurntSienna         = base.RGBColor(233, 116,  81)
    BurntUmber          = base.RGBColor(138,  51,  36)
    ButtonBlue          = base.RGBColor( 36, 160, 237)
    Byzantine           = base.RGBColor(189,  51, 164)
    Byzantium           = base.RGBColor(112,  41,  99)

    Cadet                 = base.RGBColor( 83, 104, 114)
    CadetBlue             = base.RGBColor( 95, 158, 160)
    CadetGrey             = base.RGBColor(145, 163, 176)
    CadmiumBlue           = base.RGBColor( 10,  17, 149)
    CadmiumGreen          = base.RGBColor(  0, 107,  60)
    CadmiumOrange         = base.RGBColor(237, 135,  45)
    CadmiumPurple         = base.RGBColor(182,  12,  38)
    CadmiumRed            = base.RGBColor(227,   0,  34)
    CadmiumYellow         = base.RGBColor(255, 246,   0)
    CadmiumViolet         = base.RGBColor(127,  62, 152)
    CaféAuLait            = base.RGBColor(166, 123,  91)
    CaféNoir              = base.RGBColor( 75,  54,  33)
    CalPolyPomonaGreen    = base.RGBColor( 30,  77,  43)
    Calamansi             = base.RGBColor(252, 255, 164)
    CambridgeBlue         = base.RGBColor(163, 193, 173)
    Camel                 = base.RGBColor(193, 154, 107)
    CameoPink             = base.RGBColor(239, 187, 204)
    CamouflageGreen       = base.RGBColor(120, 134, 107)
    Canary                = base.RGBColor(255, 255, 153)
    CanaryYellow          = base.RGBColor(255, 239,   0)
    CandyAppleRed         = base.RGBColor(255,   8,   0)
    CandyPink             = base.RGBColor(228, 113, 122)
    Capri                 = base.RGBColor(  0, 191, 255)
    CaputMortuum          = base.RGBColor( 89,  39,  32)
    Caramel               = base.RGBColor(255, 213, 154)
    Cardinal              = base.RGBColor(196,  30,  58)
    CaribbeanGreen        = base.RGBColor(  0, 204, 153)
    Carmine               = base.RGBColor(150,   0,  24)
    MAndPCarmine          = base.RGBColor(215,   0,  64)
    CarminePink           = base.RGBColor(235,  76,  66)
    CarmineRed            = base.RGBColor(255,   0,  56)
    CarnationPink         = base.RGBColor(255, 166, 201)
    Carnelian             = base.RGBColor(179,  27,  27)
    CarolinaBlue          = base.RGBColor( 86, 160, 211)
    CarrotOrange          = base.RGBColor(237, 145,  33)
    CastletonGreen        = base.RGBColor(  0,  86,  63)
    CatalinaBlue          = base.RGBColor(  6,  42, 120)
    Catawba               = base.RGBColor(112,  54,  66)
    CedarChest            = base.RGBColor(201,  90,  73)
    Ceil                  = base.RGBColor(146, 161, 207)
    Celadon               = base.RGBColor(172, 225, 175)
    CeladonBlue           = base.RGBColor(  0, 123, 167)
    CeladonGreen          = base.RGBColor( 47, 132, 124)
    Celeste               = base.RGBColor(178, 255, 255)
    CelestialBlue         = base.RGBColor( 73, 151, 208)
    Cerise                = base.RGBColor(222,  49,  99)
    CerisePink            = base.RGBColor(236,  59, 131)
    Cerulean              = base.RGBColor(  0, 123, 167)
    CeruleanBlue          = base.RGBColor( 42,  82, 190)
    CeruleanFrost         = base.RGBColor(109, 155, 195)
    CGBlue                = base.RGBColor(  0, 122, 165)
    CGRed                 = base.RGBColor(224,  60,  49)
    Chamoisee             = base.RGBColor(160, 120,  90)
    Champagne             = base.RGBColor(247, 231, 206)
    ChampagnePink         = base.RGBColor(241, 221, 207)
    Charcoal              = base.RGBColor( 54,  69,  79)
    CharlestonGreen       = base.RGBColor( 35,  43,  43)
    Charm                 = base.RGBColor(208, 116, 139)
    CharmPink             = base.RGBColor(230, 143, 172)
    TraditionalChartreuse = base.RGBColor(223, 255,   0)
    WebChartreuse         = base.RGBColor(127, 255,   0)
    Cheese                = base.RGBColor(255, 166,   0)
    Cherry                = base.RGBColor(222,  49,  99)
    CherryBlossomPink     = base.RGBColor(255, 183, 197)
    Chestnut              = base.RGBColor(149,  69,  53)
    ChinaPink             = base.RGBColor(222, 111, 161)
    ChinaRose             = base.RGBColor(168,  81, 110)
    ChineseBlack          = base.RGBColor( 20,  20,  20)
    ChineseBlue           = base.RGBColor( 54,  81, 148)
    ChineseBronze         = base.RGBColor(205, 128,  50)
    ChineseBrown          = base.RGBColor(171,  56,  31)
    ChineseGreen          = base.RGBColor(208, 219,  97)
    ChineseGold           = base.RGBColor(204, 153,   0)
    ChineseOrange         = base.RGBColor(243, 112,  66)
    ChinesePink           = base.RGBColor(222, 112, 161)
    ChinesePurple         = base.RGBColor(114,  11, 152)
    ChineseRed            = base.RGBColor(205,   7,  30)
    ChineseRed2           = base.RGBColor(170,  56,  30)
    ChineseSilver         = base.RGBColor(204, 204, 204)
    ChineseViolet         = base.RGBColor(133,  96, 136)
    ChineseWhite          = base.RGBColor(226, 229, 222)
    ChineseYellow         = base.RGBColor(255, 178,   0)
    ChlorophyllGreen      = base.RGBColor( 74, 255,   0)
    ChocolateKisses       = base.RGBColor( 60,  20,  33)
    TraditionalChocolate  = base.RGBColor(123,  63,   0)
    WebChocolate          = base.RGBColor(210, 105,  30)
    ChristmasBlue         = base.RGBColor( 42, 143, 189)
    ChristmasBlue2        = base.RGBColor( 54,  81, 148)
    ChristmasBrown        = base.RGBColor( 93,  43,  44)
    ChristmasBrown2       = base.RGBColor( 76,  31,   2)
    ChristmasGreen        = base.RGBColor( 60, 141,  13)
    ChristmasGreen2       = base.RGBColor(  0, 117,   2)
    ChristmasGold         = base.RGBColor(202, 169,   6)
    ChristmasOrange       = base.RGBColor(255, 102,   0)
    ChristmasOrange2      = base.RGBColor(213, 108,  43)
    ChristmasPink         = base.RGBColor(255, 204, 203)
    ChristmasPink2        = base.RGBColor(227,  66, 133)
    ChristmasPurple       = base.RGBColor(102,  51, 152)
    ChristmasPurple2      = base.RGBColor( 77,   8,  75)
    ChristmasRed          = base.RGBColor(170,   1,  20)
    ChristmasRed2         = base.RGBColor(176,  27,  46)
    ChristmasSilver       = base.RGBColor(225, 223, 224)
    ChristmasYellow       = base.RGBColor(255, 204,   0)
    ChristmasYellow2      = base.RGBColor(254, 242,   0)
    ChromeYellow          = base.RGBColor(255, 167,   0)
    Cinereous             = base.RGBColor(152, 129, 123)
    Cinnabar              = base.RGBColor(227,  66,  52)
    Cinnamon              = base.RGBColor(210, 105,  30)
    CinnamonSatin         = base.RGBColor(205,  96, 126)
    Citrine               = base.RGBColor(228, 208,  10)
    CitrineBrown          = base.RGBColor(147,  55,   9)
    Citron                = base.RGBColor(159, 169,  31)
    Claret                = base.RGBColor(127,  23,  52)
    ClassicRose           = base.RGBColor(251, 204, 231)
    CobaltBlue            = base.RGBColor(  0,  71, 171)
    CocoaBrown            = base.RGBColor(210, 105,  30)
    Coconut               = base.RGBColor(150,  90,  62)
    Coffee                = base.RGBColor(111,  78,  55)
    Cola                  = base.RGBColor( 60,  48,  36)
    ColumbiaBlue          = base.RGBColor(196, 216, 226)
    Conditioner           = base.RGBColor(255, 255, 204)
    CongoPink             = base.RGBColor(248, 131, 121)
    CoolBlack             = base.RGBColor(  0,  46,  99)
    CoolGrey              = base.RGBColor(140, 146, 172)
    CookiesAndCream       = base.RGBColor(238, 224, 177)
    Copper                = base.RGBColor(184, 115,  51)
    CrayolaCopper         = base.RGBColor(218, 138, 103)
    CopperPenny           = base.RGBColor(173, 111, 105)
    CopperRed             = base.RGBColor(203, 109,  81)
    CopperRose            = base.RGBColor(153, 102, 102)
    Coquelicot            = base.RGBColor(255,  56,   0)
    Coral                 = base.RGBColor(255, 127,  80)
    CoralPink             = base.RGBColor(248, 131, 121)
    CoralRed              = base.RGBColor(255,  64,  64)
    CoralReef             = base.RGBColor(253, 124, 110)
    Cordovan              = base.RGBColor(137,  63,  69)
    Corn                  = base.RGBColor(251, 236,  93)
    CornellRed            = base.RGBColor(179,  27,  27)
    CornflowerBlue        = base.RGBColor(100, 149, 237)
    Cornsilk              = base.RGBColor(255, 248, 220)
    CosmicCobalt          = base.RGBColor( 46,  45, 136)
    CosmicLatte           = base.RGBColor(255, 248, 231)
    CoyoteBrown           = base.RGBColor(129,  97,  60)
    CottonCandy           = base.RGBColor(255, 188, 217)
    Cream                 = base.RGBColor(255, 253, 208)
    Crimson               = base.RGBColor(220,  20,  60)
    CrimsonGlory          = base.RGBColor(190,   0,  50)
    CrimsonRed            = base.RGBColor(153,   0,   0)
    Cultured              = base.RGBColor(245, 245, 245)
    Cyan                  = base.RGBColor(  0, 255, 255)
    CyanAzure             = base.RGBColor( 78, 130, 180)
    CyanBlueAzure         = base.RGBColor( 70, 130, 191)
    CyanCobaltBlue        = base.RGBColor( 40,  88, 156)
    CyanCornflowerBlue    = base.RGBColor( 24, 139, 194)
    ProcessCyan           = base.RGBColor(  0, 183, 235)
    CyberGrape            = base.RGBColor( 88,  66, 124)
    CyberYellow           = base.RGBColor(255, 211,   0)
    Cyclamen              = base.RGBColor(245, 111, 161)
    
    Daffodil               = base.RGBColor(255, 255,  49)
    Dandelion              = base.RGBColor(240, 225,  48)
    DarkBlue               = base.RGBColor(  0,   0, 139)
    DarkBlueGray           = base.RGBColor(102, 102, 153)
    DarkBronze             = base.RGBColor(128,  74,   0)
    DarkBrown              = base.RGBColor(101,  67,  33)
    DarkBrownTangelo       = base.RGBColor(136, 101,  78)
    DarkByzantium          = base.RGBColor( 93,  57,  84)
    DarkCandyAppleRed      = base.RGBColor(164,   0,   0)
    DarkCerulean           = base.RGBColor(  8,  69, 126)
    DarkCharcoal           = base.RGBColor( 51,  51,  51)
    DarkChestnut           = base.RGBColor(152, 105,  96)
    DarkChocolate          = base.RGBColor( 73,   2,   6)
    HersheysDarkChocolate  = base.RGBColor( 60,  19,  33)
    DarkCornflowerBlue     = base.RGBColor( 38,  66, 139)
    DarkCoral              = base.RGBColor(205,  91,  69)
    DarkCyan               = base.RGBColor(  0, 139, 139)
    DarkGoldenrod          = base.RGBColor(184, 134,  11)
    X11DarkGray            = base.RGBColor(169, 169, 169)
    DarkGreen              = base.RGBColor(  1,  50,  32)
    X11DarkGreen           = base.RGBColor(  0, 100,   0)
    DarkGunmetal           = base.RGBColor( 31,  38,  42)
    DarkImperialBlue       = base.RGBColor(  0,  65, 106)
    DarkImperialBlue2      = base.RGBColor(  0,  20, 126)
    DarkJungleGreen        = base.RGBColor( 26,  36,  33)
    DarkKhaki              = base.RGBColor(189, 183, 107)
    DarkLava               = base.RGBColor( 72,  60,  50)
    DarkLavender           = base.RGBColor(115,  79, 150)
    DarkLemonLime          = base.RGBColor(139, 190,  27)
    DarkLiver              = base.RGBColor( 83,  75,  79)
    HorsesDarkLiver        = base.RGBColor( 84,  61,  55)
    DarkMagenta            = base.RGBColor(139,   0, 139)
    DarkMediumGray         = base.RGBColor(169, 169, 169)
    DarkMidnightBlue       = base.RGBColor(  0,  51, 102)
    DarkMossGreen          = base.RGBColor( 74,  93,  35)
    DarkOliveGreen         = base.RGBColor( 85, 107,  47)
    DarkOrange             = base.RGBColor(255, 140,   0)
    DarkOrchid             = base.RGBColor(153,  50, 204)
    DarkPastelBlue         = base.RGBColor(119, 158, 203)
    DarkPastelGreen        = base.RGBColor(  3, 192,  60)
    DarkPastelPurple       = base.RGBColor(150, 111, 214)
    DarkPastelRed          = base.RGBColor(194,  59,  34)
    DarkPink               = base.RGBColor(231,  84, 128)
    DarkPowderBlue         = base.RGBColor(  0,  51, 153)
    DarkPuce               = base.RGBColor( 79,  58,  60)
    DarkPurple             = base.RGBColor( 48,  25,  52)
    DarkRaspberry          = base.RGBColor(135,  38,  87)
    DarkRed                = base.RGBColor(135,  38,  87)
    DarkSalmon             = base.RGBColor(233, 150, 122)
    DarkScarlet            = base.RGBColor( 86,   3,  25)
    DarkSeaGreen           = base.RGBColor(143, 188, 143)
    DarkSienna             = base.RGBColor( 60,  20,  20)
    DarkSkyBlue            = base.RGBColor(140, 190, 214)
    DarkSlateBlue          = base.RGBColor( 72,  61, 139)
    DarkSlateGray          = base.RGBColor( 47,  79,  79)
    DarkSpringGreen        = base.RGBColor( 23, 114,  69)
    DarkTan                = base.RGBColor(145, 129,  81)
    DarkTangerine          = base.RGBColor(255, 168,  18)
    DarkTaupe              = base.RGBColor( 72,  60,  50)
    DarkTerraCotta         = base.RGBColor(204,  78,  92)
    DarkTurquoise          = base.RGBColor(  0, 206, 209)
    DarkVanilla            = base.RGBColor(209, 190, 168)
    DarkViolet             = base.RGBColor(148,   0, 211)
    DarkYellow             = base.RGBColor(155, 135,  12)
    DartmouthGreen         = base.RGBColor(  0, 112,  60)
    DavysGrey              = base.RGBColor( 85,  85,  85)
    DebianRed              = base.RGBColor(215,  10,  83)
    DeepAmethyst           = base.RGBColor(156, 138, 164)
    DeepAquamarine         = base.RGBColor( 64, 130, 109)
    DeepCarmine            = base.RGBColor(169,  32,  62)
    DeepCarminePink        = base.RGBColor(239,  48,  56)
    DeepCarrotOrange       = base.RGBColor(233, 105,  44)
    DeepCerise             = base.RGBColor(218,  50, 135)
    DeepChampagne          = base.RGBColor(250, 214, 165)
    DeepChestnut           = base.RGBColor(185,  78,  72)
    DeepCoffee             = base.RGBColor(112,  66,  65)
    DeepFuchsia            = base.RGBColor(193,  84, 193)
    DeepGreen              = base.RGBColor(  5, 102,   8)
    DeepGreenCyanTurquoise = base.RGBColor( 14, 124,  97)
    DeepJungleGreen        = base.RGBColor(  0,  75,  73)
    DeepKoamaru            = base.RGBColor( 51,  51, 102)
    DeepLemon              = base.RGBColor(245, 199,  26)
    DeepLilac              = base.RGBColor(153,  85, 187)
    DeepMagenta            = base.RGBColor(204,   0, 204)
    DeepMaroon             = base.RGBColor(130,   0,   0)
    DeepMauve              = base.RGBColor(212, 115, 212)
    DeepMossGreen          = base.RGBColor( 53,  94,  59)
    DeepPeach              = base.RGBColor(255, 203, 164)
    DeepPink               = base.RGBColor(255,  20, 147)
    DeepPuce               = base.RGBColor(169,  92, 104)
    DeepRed                = base.RGBColor(133,   1,   1)
    DeepRuby               = base.RGBColor(132,  63,  91)
    DeepSaffron            = base.RGBColor(255, 153,  51)
    DeepSkyBlue            = base.RGBColor(  0, 191, 255)
    DeepSpaceSparkle       = base.RGBColor( 74, 100, 108)
    DeepSpringBud          = base.RGBColor( 85, 107,  47)
    DeepTaupe              = base.RGBColor(126,  94,  96)
    DeepTuscanRed          = base.RGBColor(102,  66,  77)
    DeepViolet             = base.RGBColor( 51,   0, 102)
    Deer                   = base.RGBColor(186, 135,  89)
    Denim                  = base.RGBColor( 21,  96, 189)
    DenimBlue              = base.RGBColor( 34,  67, 182)
    DesaturatedCyan        = base.RGBColor(102, 153, 153)
    Desert                 = base.RGBColor(193, 154, 107)
    DesertSand             = base.RGBColor(237, 201, 175)
    Desire                 = base.RGBColor(234,  60,  83)
    Diamond                = base.RGBColor(185, 242, 255)
    DimGray                = base.RGBColor(105, 105, 105)
    DingyDungeon           = base.RGBColor(197,  49,  81)
    Dirt                   = base.RGBColor(155, 118,  83)
    DirtyBrown             = base.RGBColor(181, 101,  30)
    DirtyWhite             = base.RGBColor(232, 228, 201)
    DodgerBlue             = base.RGBColor( 30, 144, 255)
    DodieYellow            = base.RGBColor(254, 246,  91)
    DogwoodRose            = base.RGBColor(215,  24, 104)
    DollarBill             = base.RGBColor(133, 187, 101)
    DolphinGray            = base.RGBColor(130, 142, 132)
    DonkeyBrown            = base.RGBColor(102,  76,  40)
    Drab                   = base.RGBColor(150, 113,  23)
    DukeBlue               = base.RGBColor(  0,   0, 156)
    DustStorm              = base.RGBColor(229, 204, 201)
    DutchWhite             = base.RGBColor(239, 223, 187)
    
    EarthYellow         = base.RGBColor(225, 169,  95)
    Ebony               = base.RGBColor( 85,  93,  80)
    Ecru                = base.RGBColor(194, 178, 128)
    EerieBlack          = base.RGBColor( 27,  27,  27)
    Eggplant            = base.RGBColor( 97,  64,  81)
    Eggshell            = base.RGBColor(240, 234, 214)
    EgyptianBlue        = base.RGBColor( 16,  52, 166)
    ElectricBlue        = base.RGBColor(125, 249, 255)
    ElectricCrimson     = base.RGBColor(255,   0,  63)
    ElectricCyan        = base.RGBColor(  0, 255, 255)
    ElectricGreen       = base.RGBColor(  0, 255,   0)
    ElectricIndigo      = base.RGBColor(111,   0, 255)
    ElectricLavender    = base.RGBColor(244, 187, 255)
    ElectricLime        = base.RGBColor(204, 255,   0)
    ElectricPurple      = base.RGBColor(191,   0, 255)
    ElectricUltramarine = base.RGBColor( 63,   0, 255)
    ElectricViolet      = base.RGBColor(143,   0, 255)
    ElectricYellow      = base.RGBColor(255, 255,  51)
    Emerald             = base.RGBColor( 80, 200, 120)
    EmeraldGreen        = base.RGBColor(  4,  99,   7)
    Eminence            = base.RGBColor(108,  48, 130)
    EnglishGreen        = base.RGBColor( 27,  77,  62)
    EnglishLavender     = base.RGBColor(180, 131, 149)
    EnglishRed          = base.RGBColor(171,  75,  82)
    EnglishVermillion   = base.RGBColor(204,  71,  75)
    EnglishViolet       = base.RGBColor( 86,  60,  92)
    EtonBlue            = base.RGBColor(150, 200, 162)
    Eucalyptus          = base.RGBColor( 68, 215, 168)
    
    Fallow                 = base.RGBColor(193, 154, 107)
    FaluRed                = base.RGBColor(128,  24,  24)
    Fandango               = base.RGBColor(181,  51, 137)
    FandangoPink           = base.RGBColor(222,  82, 133)
    FashionFuchsia         = base.RGBColor(244,   0, 161)
    Fawn                   = base.RGBColor(229, 170, 112)
    Feldgrau               = base.RGBColor( 77,  93,  83)
    Feldspar               = base.RGBColor(253, 213, 177)
    FernGreen              = base.RGBColor( 79, 121,  66)
    FerrariRed             = base.RGBColor(255,  40,   0)
    FieldDrab              = base.RGBColor(108,  84,  30)
    FieryRose              = base.RGBColor(255,  84, 112)
    Firebrick              = base.RGBColor(178,  34,  34)
    FireEngineRed          = base.RGBColor(206,  32,  41)
    FireOpal               = base.RGBColor(233,  92,  75)
    Flame                  = base.RGBColor(226,  88,  34)
    FlamingoPink           = base.RGBColor(252, 142, 172)
    Flattery               = base.RGBColor(107,  68,  35)
    Flavescent             = base.RGBColor(247, 233, 142)
    Flax                   = base.RGBColor(238, 220, 130)
    Flesh                  = base.RGBColor(255, 233, 209)
    Flirt                  = base.RGBColor(162,   0, 109)
    FloralWhite            = base.RGBColor(255, 250, 240)
    FluorescentBlue        = base.RGBColor( 21, 244, 238)
    FluorescentOrange      = base.RGBColor(255, 191,   0)
    FluorescentPink        = base.RGBColor(255,  20, 147)
    FluorescentYellow      = base.RGBColor(204, 255,   0)
    Folly                  = base.RGBColor(255,   0,  79)
    TraditionalForestGreen = base.RGBColor(  1,  68,  33)
    WebForestGreen         = base.RGBColor( 34, 139,  34)
    FrenchBeige            = base.RGBColor(166, 123,  91)
    FrenchBistre           = base.RGBColor(133, 109,  77)
    FrenchBlue             = base.RGBColor(  0, 114, 187)
    FrenchFuchsia          = base.RGBColor(253,  63, 146)
    FrenchLilac            = base.RGBColor(134,  96, 142)
    FrenchLime             = base.RGBColor(158, 253,  56)
    FrenchMauve            = base.RGBColor(212, 115, 212)
    FrenchPink             = base.RGBColor(253, 108, 158)
    FrenchPuce             = base.RGBColor(129,  20,  83)
    FrenchRaspberry        = base.RGBColor(199,  44,  72)
    FrenchRose             = base.RGBColor(246,  74, 138)
    FrenchSkyBlue          = base.RGBColor(119, 181, 254)
    FrenchViolet           = base.RGBColor(136,   6, 206)
    FrenchWine             = base.RGBColor(172,  30,  68)
    FreshAir               = base.RGBColor(166, 231, 255)
    Frostbite              = base.RGBColor(233,  54, 167)
    Fuchsia                = base.RGBColor(255,   0, 255)
    CrayolaFuchsia         = base.RGBColor(193,  84, 193)
    FuchsiaPink            = base.RGBColor(255, 119, 255)
    FuchsiaPurple          = base.RGBColor(204,  57, 123)
    FuchsiaRose            = base.RGBColor(199,  67, 117)
    Fulvous                = base.RGBColor(228, 132,   0)
    FuzzyWuzzy             = base.RGBColor(204, 102, 102)

    Gainsboro          = base.RGBColor(220, 220, 220)
    Gamboge            = base.RGBColor(228, 155,  15)
    GambogeOrange      = base.RGBColor(153, 102,   0)
    GambogeBrown       = base.RGBColor(153, 102,   0)
    Garnet             = base.RGBColor(115,  54,  53)
    GargoyleGas        = base.RGBColor(255, 223,  70)
    GenericViridian    = base.RGBColor(  0, 127, 102)
    GhostWhite         = base.RGBColor(248, 248, 255)
    GiantsClub         = base.RGBColor(176,  92,  82)
    GiantsOrange       = base.RGBColor(254,  90,  29)
    Glaucous           = base.RGBColor( 96, 130, 182)
    GlossyGrape        = base.RGBColor(171, 146, 179)
    GOGreen            = base.RGBColor(  0, 171, 102)
    Gold               = base.RGBColor(165, 124,   0)
    MetallicGold       = base.RGBColor(212, 175,  55)
    WebGold            = base.RGBColor(255, 215,   0)
    Golden             = base.RGBColor(255, 215,   0)
    CrayolaGold        = base.RGBColor(230, 190, 138)
    GoldFusion         = base.RGBColor(133, 117,  78)
    GoldFoil           = base.RGBColor(189, 155,  22)
    GoldBrown          = base.RGBColor(153, 101,  21)
    GoldPoppy          = base.RGBColor(252,  94,   0)
    GoldYellow         = base.RGBColor(255, 223,   0)
    Goldenrod          = base.RGBColor(218, 165,  32)
    GraniteGray        = base.RGBColor(103, 103, 103)
    GrannySmithApple   = base.RGBColor(168, 228, 160)
    Grape              = base.RGBColor(111,  45, 168)
    Gray               = base.RGBColor(128, 128, 128)
    WebGray            = base.RGBColor(128, 128, 128)
    CSSGray            = base.RGBColor(128, 128, 128)
    X11Gray            = base.RGBColor(190, 190, 190)
    GrayAsparagus      = base.RGBColor( 70,  89,  69)
    GrayBlue           = base.RGBColor(140, 146, 172)
    Green              = base.RGBColor(  0, 128,   1)
    ColorWheelGreen    = base.RGBColor(  0, 255,   0)
    X11Green           = base.RGBColor(  0, 255,   0)
    CrayolaGreen       = base.RGBColor( 28, 172, 120)
    WebGreen           = base.RGBColor(  0, 128,   0)
    CSSGreen           = base.RGBColor(  0, 128,   0)
    MunsellGreen       = base.RGBColor(  0, 168, 119)
    NCSGreen           = base.RGBColor(  0, 159, 107)
    PantoneGreen       = base.RGBColor(  0, 173,  67)
    PigmentGreen       = base.RGBColor(  0, 165,  80)
    RYBGreen           = base.RGBColor(102, 176,  50)
    GreenBlue          = base.RGBColor( 17, 100, 180)
    GreenBlue          = base.RGBColor( 17, 100, 180)
    GreenCyan          = base.RGBColor(  0, 153, 102)
    GreenLizard        = base.RGBColor(167, 244,  50)
    GreenSheen         = base.RGBColor(110, 174, 161)
    GreenYellow        = base.RGBColor(173, 255,  47)
    Grullo             = base.RGBColor(169, 154, 134)
    GuppieGreen        = base.RGBColor(  0, 255, 127)
    Gunmetal           = base.RGBColor( 42,  52,  57)
    
    HalayàÚbe         = base.RGBColor(102,  56,  84)
    HalloweenOrange   = base.RGBColor(235,  97,  35)
    HanBlue           = base.RGBColor( 68, 108, 207)
    HanPurple         = base.RGBColor( 82,  24, 250)
    HansaYellow       = base.RGBColor(233, 214, 107)
    Harlequin         = base.RGBColor( 63, 255,   0)
    HarlequinGreen    = base.RGBColor( 70, 203,  24)
    HarvardCrimson    = base.RGBColor(201,   0,  22)
    HarvestGold       = base.RGBColor(218, 145,   0)
    HeartGold         = base.RGBColor(128, 128,   0)
    HeatWave          = base.RGBColor(255, 122,   0)
    HeidelbergRed     = base.RGBColor(150,   0,  24)
    Heliotrope        = base.RGBColor(223, 115, 255)
    HeliotropeGray    = base.RGBColor(170, 152, 169)
    HeliotropeMagenta = base.RGBColor(170,   0, 187)
    HollywoodCerise   = base.RGBColor(244,   0, 161)
    Honeydew          = base.RGBColor(240, 255, 240)
    HonoluluBlue      = base.RGBColor(  0, 109, 176)
    HookersGreen      = base.RGBColor( 73, 121, 107)
    HotMagenta        = base.RGBColor(255,  29, 206)
    HotPink           = base.RGBColor(255, 105, 180)
    HunterGreen       = base.RGBColor( 53,  94,  59)
    
    Iceberg                             = base.RGBColor(113, 166, 210)
    Icterine                            = base.RGBColor(252, 247,  94)
    IguanaGreen                         = base.RGBColor(113, 188, 120)
    IlluminatingEmerald                 = base.RGBColor( 49, 145, 119)
    Imperial                            = base.RGBColor( 49, 145, 119)
    ImperialBlue                        = base.RGBColor(  0,  35, 149)
    ImperialPurple                      = base.RGBColor(102,   2,  60)
    ImperialRed                         = base.RGBColor(237,  41,  57)
    Inchworm                            = base.RGBColor(178, 236,  93)
    Independence                        = base.RGBColor( 76,  81, 109)
    IndiaGreen                          = base.RGBColor( 19, 136,   8)
    IndianRed                           = base.RGBColor(205,  92,  92)
    IndianYellow                        = base.RGBColor(227, 168,  87)
    Indigo                              = base.RGBColor( 75,   0, 130)
    IndigoDye                           = base.RGBColor(  9,  31, 146)
    RainbowIndigo                       = base.RGBColor( 35,  48, 103)
    WebIndigo                           = base.RGBColor( 75,   0, 130)
    InfraRed                            = base.RGBColor(255,  73, 108)
    InterdimensionalBlue                = base.RGBColor( 54,  12, 204)
    InternationalKleinBlue              = base.RGBColor(  0,  47, 167)
    AerospaceInternationalOrange        = base.RGBColor(255,  79,   0)
    EngineeringInternationalOrange      = base.RGBColor(255,  79,   0)
    GoldenGateBridgeInternationalOrange = base.RGBColor(192,  54,  44)
    Iris                                = base.RGBColor( 90,  79, 207)
    Irresistible                        = base.RGBColor(179,  68, 108)
    Isabelline                          = base.RGBColor(244, 240, 236)
    IslamicGreen                        = base.RGBColor(  0, 144,   0)
    ItalianSkyBlue                      = base.RGBColor(178, 255, 255)
    Ivory                               = base.RGBColor(255, 255, 240)

    Jacarta         = base.RGBColor( 61,  50,  93)
    JackoBean       = base.RGBColor( 65,  54,  40)
    Jade            = base.RGBColor(  0, 168, 107)
    JapaneseCarmine = base.RGBColor(157,  41,  51)
    JapaneseIndigo  = base.RGBColor( 38,  67,  72)
    JapaneseLaurel  = base.RGBColor( 47, 117,  50)
    JapaneseViolet  = base.RGBColor( 91,  50,  86)
    Jasmine         = base.RGBColor(248, 222, 126)
    Jasper          = base.RGBColor(215,  59,  62)
    JasperOrange    = base.RGBColor(222, 143,  78)
    JazzberryJam    = base.RGBColor(165,  11,  94)
    JellyBean       = base.RGBColor(218,  97,  78)
    JellyBeanBlue   = base.RGBColor( 68, 121, 142)
    Jet             = base.RGBColor( 52,  52,  52)
    JetStream       = base.RGBColor(187, 208, 201)
    Jonquil         = base.RGBColor(244, 202,  22)
    JordyBlue       = base.RGBColor(138, 185, 241)
    JuneBud         = base.RGBColor(189, 218,  87)
    JungleGreen     = base.RGBColor( 41, 171, 135)
    
    KellyGreen   = base.RGBColor( 76, 187,  23)
    KenyanCopper = base.RGBColor(124,  28,   5)
    Keppel       = base.RGBColor( 58, 176, 158)
    KeyLime      = base.RGBColor(232, 244, 140)
    Khaki        = base.RGBColor(195, 176, 145)
    WebKhaki     = base.RGBColor(195, 176, 145)
    CSSKhaki     = base.RGBColor(195, 176, 145)
    X11Khaki     = base.RGBColor(240, 230, 140)
    Kiwi         = base.RGBColor(142, 229,  63)
    Kobe         = base.RGBColor(136,  45,  23)
    Kobi         = base.RGBColor(231, 159, 196)
    Kobicha      = base.RGBColor(107,  68,  35)
    KombuGreen   = base.RGBColor( 53,  66,  48)
    KSUPurple    = base.RGBColor( 81,  40, 136)
    KUCrimson    = base.RGBColor(232,   0,  13)

    LaSalleGreen         = base.RGBColor(  8, 120,  48)
    LanguidLavender      = base.RGBColor(214, 202, 221)
    LapisLazuli          = base.RGBColor( 38,  97, 156)
    LaserLemon           = base.RGBColor(255, 255, 102)
    LaurelGreen          = base.RGBColor(169, 186, 157)
    Lava                 = base.RGBColor(207,  16,  32)
    FloralLavender       = base.RGBColor(181, 126, 220)
    WebLavender          = base.RGBColor(230, 230, 250)
    LavenderBlue         = base.RGBColor(204, 204, 255)
    LavenderBlush        = base.RGBColor(255, 240, 245)
    LavenderGray         = base.RGBColor(196, 195, 208)
    LavenderIndigo       = base.RGBColor(148,  87, 235)
    LavenderMagenta      = base.RGBColor(238, 130, 238)
    LavenderMist         = base.RGBColor(230, 230, 250)
    LavenderPink         = base.RGBColor(251, 174, 210)
    LavenderPurple       = base.RGBColor(150, 123, 182)
    LavenderRose         = base.RGBColor(251, 160, 227)
    LawnGreen            = base.RGBColor(124, 252,   0)
    Lemon                = base.RGBColor(255, 247,   0)
    LemonChiffon         = base.RGBColor(255, 250, 205)
    LemonCurry           = base.RGBColor(204, 160,  29)
    LemonGlacier         = base.RGBColor(253, 255,   0)
    LemonMeringue        = base.RGBColor(246, 234, 190)
    LemonYellow          = base.RGBColor(255, 244,  79)
    CrayolaLemonYellow   = base.RGBColor(255, 244,  79)
    Lenurple             = base.RGBColor(186, 147, 216)
    Liberty              = base.RGBColor( 84,  90, 167)
    Licorice             = base.RGBColor( 26,  17,  16)
    LightApricot         = base.RGBColor(253, 213, 177)
    LightBlue            = base.RGBColor(173, 216, 230)
    LightBrown           = base.RGBColor(181, 101,  29)
    LightCarminePink     = base.RGBColor(230, 103, 113)
    LightCobaltBlue      = base.RGBColor(136, 172, 224)
    LightCoral           = base.RGBColor(240, 128, 128)
    LightCornflowerBlue  = base.RGBColor(240, 128, 128)
    LightCrimson         = base.RGBColor(245, 105, 145)
    LightCyan            = base.RGBColor(224, 255, 255)
    LightDeepPink        = base.RGBColor(255,  92, 205)
    LightFrenchBeige     = base.RGBColor(200, 173, 127)
    LightFuchsiaPink     = base.RGBColor(249, 132, 239)
    LightGold            = base.RGBColor(178, 151,   0)
    LightGoldenrodYellow = base.RGBColor(250, 250, 210)
    LightGray            = base.RGBColor(211, 211, 211)
    LightGrayishMagenta  = base.RGBColor(204, 153, 204)
    LightGreen           = base.RGBColor(144, 238, 144)
    LightHotPink         = base.RGBColor(255, 179, 222)
    LightKhaki           = base.RGBColor(240, 230, 140)
    LightMediumOrchid    = base.RGBColor(211, 155, 203)
    LightMossGreen       = base.RGBColor(173, 223, 173)
    LightOrange          = base.RGBColor(254, 216, 177)
    LightOrchid          = base.RGBColor(230, 168, 215)
    LightPastelPurple    = base.RGBColor(177, 156, 217)
    LightPeriwinkle      = base.RGBColor(197, 203, 225)
    LightPink            = base.RGBColor(255, 182, 193)
    LightRed             = base.RGBColor(255, 204, 203)
    LightRedOchre        = base.RGBColor(233, 116,  81)
    LightSalmon          = base.RGBColor(255, 160, 122)
    LightSalmonPink      = base.RGBColor(255, 153, 153)
    LightSeaGreen        = base.RGBColor( 32, 178, 170)
    LightSilver          = base.RGBColor(216, 216, 216)
    LightSkyBlue         = base.RGBColor(135, 206, 250)
    LightSlateGray       = base.RGBColor(119, 136, 153)
    LightSteelBlue       = base.RGBColor(176, 196, 222)
    LightTaupe           = base.RGBColor(179, 139, 109)
    LightThulianPink     = base.RGBColor(230, 143, 172)
    LightYellow          = base.RGBColor(255, 255, 224)
    Lilac                = base.RGBColor(200, 162, 200)
    LilacLuster          = base.RGBColor(174, 152, 170)
    Lime                 = base.RGBColor(191, 255,   0)
    ColorWheelLime       = base.RGBColor(191, 255,   0)
    WebLime              = base.RGBColor(  0, 255,   0)
    LimeGreen            = base.RGBColor( 50, 205,  50)
    Limerick             = base.RGBColor(157, 194,   9)
    LincolnGreen         = base.RGBColor( 25,  89,   5)
    Linen                = base.RGBColor(250, 240, 230)
    Lion                 = base.RGBColor(193, 154, 107)
    LiseranPurple        = base.RGBColor(222, 111, 161)
    LittleBoyBlue        = base.RGBColor(108, 160, 220)
    LittleGirlPink       = base.RGBColor(248, 185, 212)
    Liver                = base.RGBColor(103,  76,  71)
    DogsLiver            = base.RGBColor(184, 109,  41)
    OrganLiver           = base.RGBColor(108,  46,  31)
    LiverChestnut        = base.RGBColor(152, 116,  86)
    Livid                = base.RGBColor(102, 153, 204)
    Lotion               = base.RGBColor(254, 253, 250)
    Lumber               = base.RGBColor(255, 228, 205)
    Lust                 = base.RGBColor(230,  32,  32)

    MaastrichtBlue        = base.RGBColor(  0,  28,  61)
    MacaroniAndCheese     = base.RGBColor(255, 189, 136)
    MadderLake            = base.RGBColor(204,  51,  54)
    Magenta               = base.RGBColor(255,   0, 255)
    CrayolaMagenta        = base.RGBColor(255,  85, 163)
    DyeMagenta            = base.RGBColor(202,  31, 123)
    PantoneMagenta        = base.RGBColor(208,  65, 126)
    ProcessMagenta        = base.RGBColor(255,   0, 144)
    MagentaHaze           = base.RGBColor(159,  69, 118)
    MagentaPink           = base.RGBColor(204,  51, 139)
    MagicMint             = base.RGBColor(170, 240, 209)
    MagicPotion           = base.RGBColor(255,  68, 102)
    Magnolia              = base.RGBColor(248, 244, 255)
    Mahogany              = base.RGBColor(192,  64,   0)
    Maize                 = base.RGBColor(251, 236,  93)
    CrayolaMaize          = base.RGBColor(242, 198,  73)
    MajorelleBlue         = base.RGBColor( 96,  80, 220)
    Malachite             = base.RGBColor( 11, 218,  81)
    Manatee               = base.RGBColor(151, 154, 170)
    Mandarin              = base.RGBColor(243, 122,  72)
    MangoGreen            = base.RGBColor(150, 255,   0)
    MangoTango            = base.RGBColor(255, 130,  67)
    Mantis                = base.RGBColor(116, 195, 101)
    MardiGras             = base.RGBColor(136,   0, 133)
    Marigold              = base.RGBColor(234, 162,  33)
    CrayolaMaroon         = base.RGBColor(195,  33,  72)
    WebMaroon             = base.RGBColor(128,   0,   0)
    CSSMaroon             = base.RGBColor(128,   0,   0)
    X11Maroon             = base.RGBColor(176,  48,  96)
    Mauve                 = base.RGBColor(224, 176, 255)
    MauveTaupe            = base.RGBColor(145,  95, 109)
    Mauvelous             = base.RGBColor(239, 152, 170)
    MaximumBlue           = base.RGBColor( 71, 171, 204)
    MaximumBlueGreen      = base.RGBColor( 48, 191, 191)
    MaximumBluePurple     = base.RGBColor(172, 172, 230)
    MaximumGreen          = base.RGBColor( 94, 140,  49)
    MaximumGreenYellow    = base.RGBColor(217, 230,  80)
    MaximumPurple         = base.RGBColor(115,  51, 128)
    MaximumRed            = base.RGBColor(217,  33,  33)
    MaximumRedPurple      = base.RGBColor(166,  58, 121)
    MaximumYellow         = base.RGBColor(250, 250,  55)
    MaximumYellowRed      = base.RGBColor(242, 186,  73)
    MayGreen              = base.RGBColor( 76, 145,  65)
    MayaBlue              = base.RGBColor(115, 194, 251)
    MeatBrown             = base.RGBColor(229, 183,  59)
    MediumAquamarine      = base.RGBColor(102, 221, 170)
    MediumBlue            = base.RGBColor(  0,  0,  205)
    MediumCandyAppleRed   = base.RGBColor(226,  6,   44)
    MediumCarmine         = base.RGBColor(175, 64,   53)
    MediumChampagne       = base.RGBColor(243, 229, 171)
    MediumElectricBlue    = base.RGBColor(  3,  80, 150)
    MediumJungleGreen     = base.RGBColor( 28,  53,  45)
    MediumLavenderMagenta = base.RGBColor(221, 160, 221)
    MediumOrchid          = base.RGBColor(186,  85, 211)
    MediumPersianBlue     = base.RGBColor(  0, 103, 165)
    MediumPurple          = base.RGBColor(147, 112, 219)
    MediumRedViolet       = base.RGBColor(187,  51, 133)
    MediumRuby            = base.RGBColor(170,  64, 105)
    MediumSeaGreen        = base.RGBColor( 60, 179, 113)
    MediumSkyBlue         = base.RGBColor(128, 218, 235)
    MediumSlateBlue       = base.RGBColor(123, 104, 238)
    MediumSpringBud       = base.RGBColor(201, 220, 135)
    MediumSpringGreen     = base.RGBColor(  0, 250, 154)
    MediumTaupe           = base.RGBColor(103,  76,  71)
    MediumTurquoise       = base.RGBColor( 72, 209, 204)
    MediumTuscanRed       = base.RGBColor(121,  68,  59)
    MediumVermilion       = base.RGBColor(217,  96,  59)
    MediumVioletRed       = base.RGBColor(199,  21, 133)
    MellowApricot         = base.RGBColor(248, 184, 120)
    MellowYellow          = base.RGBColor(248, 222, 126)
    Melon                 = base.RGBColor(253, 188, 180)
    Menthol               = base.RGBColor(193, 249, 162)
    MetallicBlue          = base.RGBColor( 50,  82, 123)
    MetallicBronze2       = base.RGBColor(169, 113,  66)
    MetallicBrown         = base.RGBColor(172,  67,  19)
    MetallicGreen         = base.RGBColor( 41, 110,   1)
    MetallicOrange        = base.RGBColor(218, 104,  15)
    MetallicPink          = base.RGBColor(237, 166, 196)
    MetallicRed           = base.RGBColor(166,  44,  43)
    MetallicSeaweed       = base.RGBColor( 10, 126, 140)
    MetallicSilver        = base.RGBColor(168, 169, 173)
    MetallicSunburst      = base.RGBColor(156, 124,  56)
    MetallicViolet        = base.RGBColor( 91,  10, 145)
    MetallicYellow        = base.RGBColor(253, 204,  13)
    MexicanPink           = base.RGBColor(228,   0, 124)
    MiddleBlue            = base.RGBColor(126, 212, 230)
    MiddleBlueGreen       = base.RGBColor(141, 217, 204)
    MiddleBluePurple      = base.RGBColor(139, 114, 190)
    MiddleGrey            = base.RGBColor(139, 134, 128)
    MiddleGreen           = base.RGBColor( 77, 140,  87)
    MiddleGreenYellow     = base.RGBColor(172, 191,  96)
    MiddlePurple          = base.RGBColor(217, 130, 181)
    MiddleRed             = base.RGBColor(229, 142, 115)
    MiddleRedPurple       = base.RGBColor(165,  83,  83)
    MiddleYellow          = base.RGBColor(255, 235,   0)
    MiddleYellowRed       = base.RGBColor(236, 177, 118)
    Midnight              = base.RGBColor(112,  38, 112)
    MidnightBlue          = base.RGBColor( 25,  25, 112)
    MidnightBlue2         = base.RGBColor(  0,  70, 140)
    MidnightGreen         = base.RGBColor(  0,  73,  83)
    EagleGreen            = base.RGBColor(  0,  73,  83)
    MikadoYellow          = base.RGBColor(255, 196,  12)
    MikadoYellow          = base.RGBColor(255, 196,  12)
    Milk                  = base.RGBColor(253, 255, 245)
    MilkChocolate         = base.RGBColor(132,  86,  60)
    MimiPink              = base.RGBColor(255, 218, 233)
    Mindaro               = base.RGBColor(227, 249, 136)
    Ming                  = base.RGBColor( 54, 116, 125)
    MinionYellow          = base.RGBColor(245, 224,  80)
    Mint                  = base.RGBColor( 62, 180, 137)
    MintCream             = base.RGBColor(245, 255, 250)
    MintGreen             = base.RGBColor(152, 255, 152)
    MistyMoss             = base.RGBColor(187, 180, 119)
    MistyRose             = base.RGBColor(255, 228, 225)
    Moccasin              = base.RGBColor(250, 235, 215)
    ModeBeige             = base.RGBColor(150, 113,  23)
    Moonstone             = base.RGBColor( 58, 168, 193)
    MoonstoneBlue         = base.RGBColor(115, 169, 194)
    MordantRed19          = base.RGBColor(174,  12,   0)
    MorningBlue           = base.RGBColor(141, 163, 153)
    MossGreen             = base.RGBColor(138, 154,  91)
    MountainMeadow        = base.RGBColor(48,  186, 143)
    MountbattenPink       = base.RGBColor(153, 122, 141)
    MSUGreen              = base.RGBColor( 24,  69,  59)
    Mud                   = base.RGBColor(112,  84,  62)
    MughalGreen           = base.RGBColor( 48,  96,  48)
    Mulberry              = base.RGBColor(197,  75, 140)
    CrayolaMulberry       = base.RGBColor(200,  80, 155)
    MummysTomb            = base.RGBColor(130, 142, 132)
    Mustard               = base.RGBColor(255, 219,  88)
    MustardBrown          = base.RGBColor(205, 122,   0)
    MustardGreen          = base.RGBColor(110, 110,  48)
    MustardYellow         = base.RGBColor(225, 173,   1)
    MyrtleGreen           = base.RGBColor( 49, 120, 115)
    Mystic                = base.RGBColor(214,  82, 130)
    MysticMaroon          = base.RGBColor(173,  67, 121)
    MysticRed             = base.RGBColor(255,  85,   0)

    NadeshikoPink   = base.RGBColor(246, 173, 198)
    NapierGreen     = base.RGBColor( 42, 128,   0)
    NapierYellow    = base.RGBColor(250, 218,  94)
    NavajoWhite     = base.RGBColor(255, 222, 173)
    Navy            = base.RGBColor(  0,   0, 128)
    NavyBlue        = base.RGBColor(  0,   0, 128)
    CrayolaNavyBlue = base.RGBColor( 25, 116, 210)
    NavyPurple      = base.RGBColor(148,  87, 235)
    NeonBlue        = base.RGBColor( 27,   3, 163)
    NeonBrown       = base.RGBColor(195, 115,  42)
    NeonCarrot      = base.RGBColor(255, 163,  67)
    NeonCyan        = base.RGBColor(  0, 254, 252)
    NeonFuchsia     = base.RGBColor(254,  65, 100)
    NeonGold        = base.RGBColor(207, 170,   1)
    NeonGray        = base.RGBColor(128, 128, 128)
    NeonGreen       = base.RGBColor( 57, 255,  20)
    NeonPink        = base.RGBColor(254,  52, 126)
    NeonPurple      = base.RGBColor(254,  52, 126)
    NeonRed         = base.RGBColor(255,  24,  24)
    NeonScarlet     = base.RGBColor(255,  38,   3)
    NeonSilver      = base.RGBColor(204, 204, 204)
    NeonTangerine   = base.RGBColor(246, 137,  10)
    NeonYellow      = base.RGBColor(255, 247,   0)
    NewCar          = base.RGBColor( 33,  79, 198)
    NewYorkPink     = base.RGBColor(215, 131, 127)
    Nickel          = base.RGBColor(114, 116, 114)
    NonPhotoBlue    = base.RGBColor(164, 221, 237)
    NorthTexasGreen = base.RGBColor(  5, 144,  51)
    Nyanza          = base.RGBColor(233, 255, 219)
    
    OceanBlue           = base.RGBColor( 79,  66, 181)
    OceanBoatBlue       = base.RGBColor(  0, 119, 190)
    OceanGreen          = base.RGBColor( 72, 191, 145)
    Ochre               = base.RGBColor(204, 119,  34)
    OfficeGreen         = base.RGBColor(  0, 128,   0)
    OgreOdor            = base.RGBColor(253,  82,  64)
    OldBurgundy         = base.RGBColor( 67,  48,  46)
    OldGold             = base.RGBColor(207, 181,  59)
    OldHeliotrope       = base.RGBColor( 86,  60,  92)
    OldLace             = base.RGBColor(253, 245, 230)
    OldLavender         = base.RGBColor(121, 104, 120)
    OldMauve            = base.RGBColor(103,  49,  71)
    OldMossGreen        = base.RGBColor(134, 126,  54)
    OldRose             = base.RGBColor(192, 128, 129)
    OldSilver           = base.RGBColor(132, 132, 130)
    Olive               = base.RGBColor(128, 128,   0)
    OliveDrab           = base.RGBColor(107, 142,  35)
    OliveDrab3          = base.RGBColor(107, 142,  35)
    OliveDrab7          = base.RGBColor( 60,  52,  31)
    OliveDrabCamouflage = base.RGBColor( 84,  79,  61)
    Olivine             = base.RGBColor(154, 185, 115)
    Onyx                = base.RGBColor( 53,  56,  57)
    Opal                = base.RGBColor(168, 195, 188)
    OperaMauve          = base.RGBColor(183, 132, 167)
    Orange              = base.RGBColor(255, 127,   0)
    ColorWheelOrange    = base.RGBColor(255, 127,   0)
    CrayolaOrange       = base.RGBColor(255, 117,  56)
    PantoneOrange       = base.RGBColor(255,  88,   0)
    RYBOrange           = base.RGBColor(251, 153,   2)
    WebOrange           = base.RGBColor(255, 165,   0)
    OrangePeel          = base.RGBColor(255, 159,   0)
    OrangeRed           = base.RGBColor(255,  69,   0)
    OrangeSoda          = base.RGBColor(250,  91,  61)
    OrangeYellow        = base.RGBColor(248, 213, 104)
    Orchid              = base.RGBColor(218, 112, 214)
    OrchidPink          = base.RGBColor(242, 189, 205)
    OriolesOrange       = base.RGBColor(251,  79,  20)
    OtterBrown          = base.RGBColor(101,  67,  33)
    OuterSpace          = base.RGBColor( 65,  74,  76)
    OutrageousOrange    = base.RGBColor(255, 110,  74)
    Oxblood             = base.RGBColor(128,   0,  32)
    OxfordBlue          = base.RGBColor(  0,  33,  71)
    Oxley               = base.RGBColor(109, 154, 121)
    OUCrimsonRed        = base.RGBColor(153,   0,   0)
    
    PacificBlue            = base.RGBColor( 28, 169, 201)
    PakistanGreen          = base.RGBColor(  0, 102,   0)
    PalatinateBlue         = base.RGBColor( 39,  59, 226)
    PalatinatePurple       = base.RGBColor(104,  40,  96)
    PaleAqua               = base.RGBColor(188, 212, 230)
    PaleBlue               = base.RGBColor(175, 238, 238)
    PaleBrown              = base.RGBColor(152, 118,  84)
    PaleCarmine            = base.RGBColor(175,  64,  53)
    PaleCerulean           = base.RGBColor(155, 196, 226)
    PaleChestnut           = base.RGBColor(221, 173, 175)
    PaleCopper             = base.RGBColor(218, 138, 103)
    PaleCornflowerBlue     = base.RGBColor(171, 205, 239)
    PaleCyan               = base.RGBColor(135, 211, 248)
    PaleGold               = base.RGBColor(230, 190, 138)
    PaleGoldenrod          = base.RGBColor(238, 232, 170)
    PaleGreen              = base.RGBColor(152, 251, 152)
    PaleLavender           = base.RGBColor(220, 208, 255)
    PaleMagenta            = base.RGBColor(249, 132, 229)
    PaleMagentaPink        = base.RGBColor(255, 153, 204)
    PalePink               = base.RGBColor(250, 218, 221)
    PalePlum               = base.RGBColor(221, 160, 221)
    PaleRedViolet          = base.RGBColor(219, 112, 147)
    PaleRobinEggBlue       = base.RGBColor(150, 222, 209)
    PaleSilver             = base.RGBColor(201, 192, 187)
    PaleSpringBud          = base.RGBColor(236, 235, 189)
    PaleTaupe              = base.RGBColor(188, 152, 126)
    PaleTurquoise          = base.RGBColor(175, 238, 238)
    PaleViolet             = base.RGBColor(204, 153, 255)
    PaleVioletRed          = base.RGBColor(219, 112, 147)
    PalmLeaf               = base.RGBColor(111, 153,  64)
    PansyPurple            = base.RGBColor(120,  24,  74)
    PaoloVeroneseGreen     = base.RGBColor(  0, 155, 125)
    PapayaWhip             = base.RGBColor(255, 239, 213)
    ParadisePink           = base.RGBColor(230,  62,  98)
    ParisGreen             = base.RGBColor( 80, 200, 120)
    ParrotPink             = base.RGBColor(217, 152, 160)
    PastelBlue             = base.RGBColor(174, 198, 207)
    PastelBrown            = base.RGBColor(131, 105,  83)
    PastelGray             = base.RGBColor(207, 207, 196)
    PastelGreen            = base.RGBColor(119, 221, 119)
    PastelMagenta          = base.RGBColor(244, 154, 194)
    PastelOrange           = base.RGBColor(255, 179,  71)
    PastelPink             = base.RGBColor(222, 165, 164)
    PastelPurple           = base.RGBColor(179, 158, 181)
    PastelRed              = base.RGBColor(255, 105,  97)
    PastelViolet           = base.RGBColor(203, 153, 201)
    PastelYellow           = base.RGBColor(253, 253, 150)
    Patriarch              = base.RGBColor(128,   0, 128)
    PaynesGrey             = base.RGBColor( 83, 104, 120)
    Peach                  = base.RGBColor(255, 229, 180)
    PeachOrange            = base.RGBColor(255, 204, 153)
    PeachPuff              = base.RGBColor(255, 218, 185)
    PeachYellow            = base.RGBColor(250, 223, 173)
    Pear                   = base.RGBColor(209, 226,  49)
    Pearl                  = base.RGBColor(234, 224, 200)
    PearlAqua              = base.RGBColor(136, 216, 192)
    PearlyPurple           = base.RGBColor(183, 104, 162)
    Peridot                = base.RGBColor(230, 226,   0)
    Periwinkle             = base.RGBColor(204, 204, 255)
    CrayolaPeriwinkle      = base.RGBColor(195, 205, 230)
    PermanentGeraniumLake  = base.RGBColor(225,  44,  44)
    PersianBlue            = base.RGBColor( 28,  57, 187)
    PersianGreen           = base.RGBColor(  0, 166, 147)
    PersianIndigo          = base.RGBColor( 50,  18, 122)
    PersianOrange          = base.RGBColor(217, 144,  88)
    PersianPink            = base.RGBColor(247, 127, 190)
    PersianPlum            = base.RGBColor(112,  28,  28)
    PersianRed             = base.RGBColor(204,  51,  51)
    PersianRose            = base.RGBColor(254,  40, 162)
    Persimmon              = base.RGBColor(236,  88,   0)
    Peru                   = base.RGBColor(205, 133,  63)
    PewterBlue             = base.RGBColor(139, 168, 183)
    PhilippineBlue         = base.RGBColor(  0,  56, 167)
    PhilippineBrown        = base.RGBColor( 93,  25,  22)
    PhilippineGold         = base.RGBColor(177, 115,   4)
    PhilippineGoldenYellow = base.RGBColor(255, 223,   0)
    PhilippineGray         = base.RGBColor(140, 140, 140)
    PhilippineGreen        = base.RGBColor(  0, 133,  67)
    PhilippineOrange       = base.RGBColor(255, 115,   0)
    PhilippinePink         = base.RGBColor(250,  26, 142)
    PhilippineRed          = base.RGBColor(206,  17,  39)
    PhilippineSilver       = base.RGBColor(179, 179, 179)
    PhilippineViolet       = base.RGBColor(129,   0, 127)
    PhilippineYellow       = base.RGBColor(254, 203,   0)
    Phlox                  = base.RGBColor(223,   0, 255)
    PhthaloBlue            = base.RGBColor(0,    15, 137)
    PhthaloGreen           = base.RGBColor( 18,  53,  36)
    PictonBlue             = base.RGBColor( 69, 177, 232)
    PictorialCarmine       = base.RGBColor(195,  11,  78)
    PiggyPink              = base.RGBColor(253, 221, 230)
    PineGreen              = base.RGBColor(  1, 121, 111)
    PineTree               = base.RGBColor( 42,  47,  35)
    Pineapple              = base.RGBColor( 86,  60,  13)
    Pink                   = base.RGBColor(255, 192, 203)
    PantonePink            = base.RGBColor(215,  72, 148)
    PinkFlamingo           = base.RGBColor(252, 116, 253)
    PinkLace               = base.RGBColor(255, 221, 244)
    PinkLavender           = base.RGBColor(216, 178, 209)
    PinkOrange             = base.RGBColor(255, 153, 102)
    PinkPearl              = base.RGBColor(231, 172, 207)
    PinkRaspberry          = base.RGBColor(152,   0,  54)
    PinkSherbet            = base.RGBColor(247, 143, 167)
    Pistachio              = base.RGBColor(147, 197, 114)
    PixiePowder            = base.RGBColor( 57,  18, 133)
    Platinum               = base.RGBColor(229, 228, 226)
    Plum                   = base.RGBColor(142,  69, 133)
    WebPlum                = base.RGBColor(221, 160, 221)
    PlumpPurple            = base.RGBColor( 89,  70, 178)
    PoliceBlue             = base.RGBColor( 55,  79, 107)
    PolishedPine           = base.RGBColor( 93, 164, 147)
    PompAndPower           = base.RGBColor(134,  96, 142)
    Popstar                = base.RGBColor(190,  79,  98)
    PortlandOrange         = base.RGBColor(255,  90,  54)
    PowderBlue             = base.RGBColor(176, 224, 230)
    PrincessPerfume        = base.RGBColor(255, 133, 207)
    PrincetonOrange        = base.RGBColor(245, 128,  37)
    Prune                  = base.RGBColor(112,  28,  28)
    PrussianBlue           = base.RGBColor(  0,  49,  83)
    PsychedelicPurple      = base.RGBColor(223,   0, 255)
    Puce                   = base.RGBColor(204, 136, 153)
    PuceRed                = base.RGBColor(114,  47,  55)
    PullmanBrown           = base.RGBColor(100,  65,  23)
    UPSBrown               = base.RGBColor(100,  65,  23)
    PullmanGreen           = base.RGBColor( 59,  51,  28)
    Pumpkin                = base.RGBColor(255, 117,  24)
    Purple                 = base.RGBColor(128,   0, 128)
    WebPurple              = base.RGBColor(128,   0, 128)
    MunsellPurple          = base.RGBColor(159,   0, 197)
    X11Purple              = base.RGBColor(160,  32, 240)
    PurpleHeart            = base.RGBColor(105,  53, 156)
    PurpleMountainMajesty  = base.RGBColor(150, 120, 182)
    PurpleNavy             = base.RGBColor( 78,  81, 128)
    PurplePizzazz          = base.RGBColor(254,  78, 218)
    PurplePlum             = base.RGBColor(156,  81, 182)
    PurpleTaupe            = base.RGBColor( 80,  64,  77)
    Purpureus              = base.RGBColor(154,  78, 174)

    Quartz              = base.RGBColor( 81,  72,  79)
    QueenBlue           = base.RGBColor( 67, 107, 149)
    QueenPink           = base.RGBColor(232, 204, 215)
    QuickSilver         = base.RGBColor(166, 166, 166)
    QuinacridoneMagenta = base.RGBColor(142,  58,  89)
    Quincy              = base.RGBColor(106,  84,  69)
    
    Rackley               = base.RGBColor( 93, 138, 168)
    RadicalRed            = base.RGBColor(255,  53,  94)
    RaisinBlack           = base.RGBColor( 36,  33,  36)
    Rajah                 = base.RGBColor(251, 171,  96)
    Raspberry             = base.RGBColor(227,  11,  93)
    RaspberryGlace        = base.RGBColor(145,  95, 109)
    RaspberryPink         = base.RGBColor(226,  80, 152)
    RaspberryRose         = base.RGBColor(179,  68, 108)
    RawSienna             = base.RGBColor(214, 138,  89)
    RawUmber              = base.RGBColor(130, 102,  68)
    RazzleDazzleRose      = base.RGBColor(255,  51, 204)
    Razzmatazz            = base.RGBColor(227,  37, 107)
    RazzmicBerry          = base.RGBColor(141,  78, 133)
    RebeccaPurple         = base.RGBColor(102,  51, 153)
    Red                   = base.RGBColor(255,   0,   0)
    CrayolaRed            = base.RGBColor(238,  32,  77)
    MunsellRed            = base.RGBColor(242,   0,  60)
    NCSRed                = base.RGBColor(196,   2,  51)
    PantoneRed            = base.RGBColor(237,  41,  57)
    PigmentRed            = base.RGBColor(237,  28,  36)
    RYBRed                = base.RGBColor(254,  39,  18)
    RedBrown              = base.RGBColor(165,  42,  42)
    RedDevil              = base.RGBColor(134,   1,  17)
    RedOrange             = base.RGBColor(255,  83,  73)
    RedPurple             = base.RGBColor(228,   0, 120)
    RedSalsa              = base.RGBColor(253,  58,  74)
    RedViolet             = base.RGBColor(199,  21, 133)
    Redwood               = base.RGBColor(164,  90,  82)
    Regalia               = base.RGBColor( 82,  45, 128)
    RegistrationBlack     = base.RGBColor(  0,   0,   0)
    ResolutionBlue        = base.RGBColor(  0,  35, 135)
    Rhythm                = base.RGBColor(119, 118, 150)
    RichBlack             = base.RGBColor(  0,  64,  64)
    FOGRA29RichBlack      = base.RGBColor(  1,  11,  19)
    FOGRA39RichBlack      = base.RGBColor(  1,   2,   3)
    RichBrilliantLavender = base.RGBColor(241, 167, 254)
    RichCarmine           = base.RGBColor(215,   0,  64)
    RichElectricBlue      = base.RGBColor(  8, 146, 208)
    RichLavender          = base.RGBColor(167, 107, 207)
    RichLilac             = base.RGBColor(182, 102, 210)
    RichMaroon            = base.RGBColor(176,  48,  96)
    RifleGreen            = base.RGBColor( 68,  76,  56)
    RoastCoffee           = base.RGBColor(112,  66,  65)
    RobinEggBlue          = base.RGBColor(  0, 204, 204)
    RocketMetallic        = base.RGBColor(138, 127, 128)
    RomanSilver           = base.RGBColor(131, 137, 150)
    RootBeer              = base.RGBColor( 41,  14,   5)
    Rose                  = base.RGBColor(255,   0, 127)
    RoseBonbon            = base.RGBColor(249,  66, 158)
    RoseDust              = base.RGBColor(158,  94, 111)
    RoseEbony             = base.RGBColor(103,  72,  70)
    RoseGarnet            = base.RGBColor(150,   1,  69)
    RoseGold              = base.RGBColor(183, 110, 121)
    RoseMadder            = base.RGBColor(227,  38,  54)
    RosePink              = base.RGBColor(255, 102, 204)
    RoseQuartz            = base.RGBColor(170, 152, 169)
    RoseQuartzPink        = base.RGBColor(189,  85, 156)
    RoseRed               = base.RGBColor(194,  30,  86)
    RoseTaupe             = base.RGBColor(144,  93,  93)
    RoseVale              = base.RGBColor(171,  78,  82)
    Rosewood              = base.RGBColor(101,   0,  11)
    RossoCorsa            = base.RGBColor(212,   0,   0)
    RosyBrown             = base.RGBColor(188, 143, 143)
    RoyalAzure            = base.RGBColor(  0,  56, 168)
    RoyalBlue             = base.RGBColor(  0,  35, 102)
    RoyalBlue2            = base.RGBColor( 65, 105, 225)
    RoyalBrown            = base.RGBColor( 82,  59,  53)
    RoyalFuchsia          = base.RGBColor(202,  44, 146)
    RoyalGreen            = base.RGBColor( 19,  98,   7)
    RoyalOrange           = base.RGBColor(249, 146,  69)
    RoyalPink             = base.RGBColor(231,  56, 149)
    RoyalRed              = base.RGBColor(155,  28,  49)
    RoyalRed2             = base.RGBColor(208,   0,  96)
    RoyalPurple           = base.RGBColor(120,  81, 169)
    RoyalYellow           = base.RGBColor(250, 218,  94)
    Ruber                 = base.RGBColor(206,  70, 118)
    RubineRed             = base.RGBColor(209,   0,  86)
    Ruby                  = base.RGBColor(224,  17,  95)
    RubyRed               = base.RGBColor(155,  17,  30)
    Ruddy                 = base.RGBColor(255,   0,  40)
    RuddyBrown            = base.RGBColor(187, 101,  40)
    RuddyPink             = base.RGBColor(225, 142, 150)
    Rufous                = base.RGBColor(168,  28,   7)
    Russet                = base.RGBColor(128,  70,  27)
    RussianGreen          = base.RGBColor(103, 146, 103)
    RussianViolet         = base.RGBColor( 50,  23,  77)
    Rust                  = base.RGBColor(183,  65,  14)
    RustyRed              = base.RGBColor(218,  44,  67)

    SacramentoStateGreen = base.RGBColor(  4,  57,  39)
    SaddleBrown          = base.RGBColor(139,  69,  19)
    SafetyOrange         = base.RGBColor(255, 120,   0)
    BlazeOrange          = base.RGBColor(255, 103,   0)
    SafetyYellow         = base.RGBColor(238, 210,   2)
    Saffron              = base.RGBColor(244, 196,  48)
    Sage                 = base.RGBColor(188, 184, 138)
    StPatricksBlue       = base.RGBColor( 35,  41, 122)
    Salem                = base.RGBColor( 23, 123,  77)
    Salmon               = base.RGBColor(250, 128, 114)
    SalmonPink           = base.RGBColor(255, 145, 164)
    Sand                 = base.RGBColor(194, 178, 128)
    SandDune             = base.RGBColor(150, 113,  23)
    Sandstorm            = base.RGBColor(236, 213,  64)
    SandyBrown           = base.RGBColor(244, 164,  96)
    SandyTan             = base.RGBColor(253, 217, 181)
    SandyTaupe           = base.RGBColor(150, 113,  23)
    Sangria              = base.RGBColor(146,   0,  10)
    SapGreen             = base.RGBColor( 80, 125,  42)
    Sapphire             = base.RGBColor( 15,  82, 186)
    SapphireBlue         = base.RGBColor(  0, 103, 165)
    SasquatchSocks       = base.RGBColor(255,  70, 129)
    SatinSheenGold       = base.RGBColor(203, 161,  53)
    Scarlet              = base.RGBColor(255,  36,   0)
    TorchRed             = base.RGBColor(253,  14,  53)
    SchaussPink          = base.RGBColor(255, 145, 175)
    SchoolBusYellow      = base.RGBColor(255, 216,   0)
    ScreaminGreen        = base.RGBColor(102, 255, 102)
    SeaBlue              = base.RGBColor(  0, 105, 148)
    SeaFoamGreen         = base.RGBColor(159, 226, 191)
    SeaGreen             = base.RGBColor( 46, 139,  87)
    CrayolaSeaGreen      = base.RGBColor(  0, 255, 205)
    SeaSerpent           = base.RGBColor( 75, 199, 207)
    SealBrown            = base.RGBColor( 89,  38,  11)
    Seashell             = base.RGBColor(255, 245, 238)
    SelectiveYellow      = base.RGBColor(255, 186,   0)
    Sepia                = base.RGBColor(112,  66,  20)
    Shadow               = base.RGBColor(138, 121,  93)
    ShadowBlue           = base.RGBColor(119, 139, 165)
    Shampoo              = base.RGBColor(255, 207, 241)
    ShamrockGreen        = base.RGBColor(  0, 158,  96)
    SheenGreen           = base.RGBColor(143, 212,   0)
    ShimmeringBlush      = base.RGBColor(217, 134, 149)
    ShinyShamrock        = base.RGBColor( 95, 167, 120)
    ShockingPink         = base.RGBColor(252,  15, 192)
    CrayolaShockingPink  = base.RGBColor(255, 111, 255)
    Sienna               = base.RGBColor(136,  45,  23)
    Silver               = base.RGBColor(192, 192, 192)
    CrayolaSilver        = base.RGBColor(201, 192, 187)
    MetallicSilver       = base.RGBColor(170, 169, 173)
    SilverChalice        = base.RGBColor(172, 172, 172)
    SilverFoil           = base.RGBColor(175, 177, 174)
    SilverLakeBlue       = base.RGBColor( 93, 137, 186)
    SilverPink           = base.RGBColor(196, 174, 173)
    SilverSand           = base.RGBColor(191, 193, 194)
    Sinopia              = base.RGBColor(203,  65,  11)
    SizzlingRed          = base.RGBColor(255,  56,  85)
    SizzlingSunrise      = base.RGBColor(255, 219,   0)
    Skobeloff            = base.RGBColor(  0, 116, 116)
    SkyBlue              = base.RGBColor(135, 206, 235)
    CrayolaSkyBlue       = base.RGBColor(118, 215, 234)
    SkyMagenta           = base.RGBColor(207, 113, 175)
    SlateBlue            = base.RGBColor(106,  90, 205)
    SlateGray            = base.RGBColor(112, 128, 144)
    SlimyGreen           = base.RGBColor( 41, 150,  23)
    Smalt                = base.RGBColor(  0,  51, 153)
    SmashedPumpkin       = base.RGBColor(255, 109,  58)
    Smitten              = base.RGBColor(200,  65, 134)
    Smoke                = base.RGBColor(115, 130, 118)
    SmokeyTopaz          = base.RGBColor(131,  42,  13)
    SmokyBlack           = base.RGBColor( 16,  12,   8)
    SmokyTopaz           = base.RGBColor(147,  61,  65)
    Snow                 = base.RGBColor(255, 250, 250)
    Soap                 = base.RGBColor(206, 200, 239)
    SoldierGreen         = base.RGBColor( 84,  90,  44)
    SolidPink            = base.RGBColor(137,  56,  67)
    SonicSilver          = base.RGBColor(117, 117, 117)
    SpartanCrimson       = base.RGBColor(158,  19,  22)
    SpaceCadet           = base.RGBColor( 29,  41,  81)
    SpanishBistre        = base.RGBColor(128, 117,  50)
    SpanishBlue          = base.RGBColor(  0, 112, 184)
    SpanishCarmine       = base.RGBColor(209,   0,  71)
    SpanishCrimson       = base.RGBColor(229,  26,  76)
    SpanishGray          = base.RGBColor(152, 152, 152)
    SpanishGreen         = base.RGBColor(  0, 145,  80)
    SpanishOrange        = base.RGBColor(232,  97,   0)
    SpanishPink          = base.RGBColor(247, 191, 190)
    SpanishPurple        = base.RGBColor(102,   3,  60)
    SpanishRed           = base.RGBColor(230,   0,  38)
    SpanishSkyBlue       = base.RGBColor(  0, 255, 255)
    SpanishViolet        = base.RGBColor( 76,  40, 130)
    SpanishViridian      = base.RGBColor(  0, 127,  92)
    SpanishYellow        = base.RGBColor(246, 181,  17)
    SpicyMix             = base.RGBColor(139,  95,  77)
    SpiroDiscoBall       = base.RGBColor( 15, 192, 252)
    SpringBud            = base.RGBColor(167, 252,   0)
    SpringFrost          = base.RGBColor(135, 255,  42)
    SpringGreen          = base.RGBColor(  0, 255, 127)
    CrayolaSpringGreen   = base.RGBColor(236, 235, 189)
    StarCommandBlue      = base.RGBColor(  0, 123, 184)
    SteelBlue            = base.RGBColor( 70, 130, 180)
    SteelPink            = base.RGBColor(204,  51, 204)
    SteelTeal            = base.RGBColor( 95, 138, 139)
    StilDeGrainYellow    = base.RGBColor(250, 218,  94)
    Stizza               = base.RGBColor(153,   0,   0)
    Stormcloud           = base.RGBColor( 79, 102, 106)
    Straw                = base.RGBColor(228, 217, 111)
    Strawberry           = base.RGBColor(252,  90, 141)
    SugarPlum            = base.RGBColor(145,  78, 117)
    SunburntCyclops      = base.RGBColor(255,  64,  76)
    Sunglow              = base.RGBColor(255, 204,  51)
    Sunny                = base.RGBColor(242, 242, 122)
    Sunray               = base.RGBColor(227, 171,  87)
    Sunset               = base.RGBColor(250, 214, 165)
    SunsetOrange         = base.RGBColor(253,  94,  83)
    SuperPink            = base.RGBColor(207, 107, 169)
    SweetBrown           = base.RGBColor(168,  55,  49)

    Tan                = base.RGBColor(210, 180, 140)
    Tangelo            = base.RGBColor(249,  77,   0)
    Tangerine          = base.RGBColor(242, 133,   0)
    TangerineYellow    = base.RGBColor(255, 204,   0)
    TangoPink          = base.RGBColor(228, 113, 122)
    TartOrange         = base.RGBColor(251,  77,  70)
    Taupe              = base.RGBColor( 72,  60,  50)
    TaupeGray          = base.RGBColor(139, 133, 137)
    TeaGreen           = base.RGBColor(208, 240, 192)
    TeaRose            = base.RGBColor(248, 131, 121)
    TeaRose2           = base.RGBColor(244, 194, 194)
    Teal               = base.RGBColor(  0, 128, 128)
    TealBlue           = base.RGBColor( 54, 117, 136)
    TealDeer           = base.RGBColor(153, 230, 179)
    TealGreen          = base.RGBColor(  0, 130, 127)
    Telemagenta        = base.RGBColor(207,  52, 118)
    Temptress          = base.RGBColor( 60,  33,  38)
    Tenné              = base.RGBColor(205,  87,   0)
    Tawny              = base.RGBColor(205,  87,   0)
    TerraCotta         = base.RGBColor(226, 114,  91)
    Thistle            = base.RGBColor(216, 191, 216)
    ThulianPink        = base.RGBColor(222, 111, 161)
    TickleMePink       = base.RGBColor(252, 137, 172)
    TiffanyBlue        = base.RGBColor( 10, 186, 181)
    TigersEye          = base.RGBColor(224, 141,  60)
    Timberwolf         = base.RGBColor(219, 215, 210)
    Titanium           = base.RGBColor(135, 134, 129)
    TitaniumYellow     = base.RGBColor(238, 230,   0)
    Tomato             = base.RGBColor(255,  99,  71)
    Toolbox            = base.RGBColor(116, 108, 192)
    Topaz              = base.RGBColor(255, 200, 124)
    TractorRed         = base.RGBColor(253,  14,  53)
    TrolleyGrey        = base.RGBColor(128, 128, 128)
    TropicalRainForest = base.RGBColor(  0, 117,  94)
    TropicalViolet     = base.RGBColor(205, 164, 222)
    TrueBlue           = base.RGBColor(  0, 115, 207)
    TuftsBlue          = base.RGBColor( 62, 142, 222)
    Tulip              = base.RGBColor(255, 135, 141)
    Tumbleweed         = base.RGBColor(222, 170, 136)
    TurkishRose        = base.RGBColor(181, 114, 129)
    Turquoise          = base.RGBColor( 64, 224, 208)
    TurquoiseBlue      = base.RGBColor(  0, 255, 239)
    TurquoiseGreen     = base.RGBColor(160, 214, 180)
    TurquoiseSurf      = base.RGBColor(  0, 197, 205)
    TurtleGreen        = base.RGBColor(138, 154,  91)
    Tuscan             = base.RGBColor(250, 214, 165)
    TuscanBrown        = base.RGBColor(111,  78,  55)
    TuscanRed          = base.RGBColor(124,  72,  72)
    TuscanTan          = base.RGBColor(166, 123,  91)
    Tuscany            = base.RGBColor(192, 153, 153)
    TwilightLavender   = base.RGBColor(138,  73, 107)
    TyrianPurple       = base.RGBColor(102,   2,  60)
    
    UABlue                      = base.RGBColor(  0,  51, 170)
    UARed                       = base.RGBColor(217,   0,  76)
    Ube                         = base.RGBColor(136, 120, 195)
    UCLABlue                    = base.RGBColor( 83, 104, 149)
    UCLAGold                    = base.RGBColor(255, 179,   0)
    UERed                       = base.RGBColor(186,   0,   1)
    UFOGreen                    = base.RGBColor( 60, 208, 112)
    Ultramarine                 = base.RGBColor( 63,   0, 255)
    UltramarineBlue             = base.RGBColor( 65, 102, 245)
    UltraPink                   = base.RGBColor(255, 111, 255)
    UltraRed                    = base.RGBColor(252, 108, 133)
    Umber                       = base.RGBColor( 99,  81,  71)
    UnbleachedSilk              = base.RGBColor(255, 221, 202)
    UnitedNationsBlue           = base.RGBColor( 91, 146, 229)
    UniversityOfCaliforniaGold  = base.RGBColor(183, 135,  39)
    UniversityOfTennesseeOrange = base.RGBColor(247, 127,   0)
    UnmellowYellow              = base.RGBColor(255, 255, 102)
    UPForestGreen               = base.RGBColor(  1,  68,  33)
    UPMaroon                    = base.RGBColor(123,  17,  19)
    UpsdellRed                  = base.RGBColor(174,  32,  41)
    Urobilin                    = base.RGBColor(225, 173,  33)
    USAFABlue                   = base.RGBColor(  0,  79, 152)
    USCCardinal                 = base.RGBColor(153,   0,   0)
    USCGold                     = base.RGBColor(255, 204,   0)
    UtahCrimson                 = base.RGBColor(211,   0,  63)
    
    VampireBlack            = base.RGBColor(  8,   8,   8)
    VanDykeBrown            = base.RGBColor(102,  66,  40)
    Vanilla                 = base.RGBColor(243, 229, 171)
    VanillaIce              = base.RGBColor(243, 143, 169)
    VegasGold               = base.RGBColor(197, 179,  88)
    VenetianRed             = base.RGBColor(200,   8,  21)
    Verdigris               = base.RGBColor( 67, 179, 174)
    Vermilion               = base.RGBColor(227,  66,  52)
    MediumVermilion2        = base.RGBColor(217,  56,  30)
    Veronica                = base.RGBColor(160,  32, 240)
    VerseGreen              = base.RGBColor( 24, 136,  13)
    VeryLightAzure          = base.RGBColor(116, 187, 251)
    VeryLightBlue           = base.RGBColor(102, 102, 255)
    VeryLightMalachiteGreen = base.RGBColor(100, 233, 134)
    VeryLightTangelo        = base.RGBColor(255, 176, 119)
    VeryPaleOrange          = base.RGBColor(255, 223, 191)
    VeryPaleYellow          = base.RGBColor(255, 255, 191)
    Violet                  = base.RGBColor(143,   0, 255)
    ColorWheelViolet        = base.RGBColor(127,   0, 255)
    CrayolaViolet           = base.RGBColor(150,  61, 127)
    RYBViolet               = base.RGBColor(134,   1, 175)
    WebViolet               = base.RGBColor(238, 130, 238)
    VioletBlue              = base.RGBColor( 50,  74, 178)
    VioletRed               = base.RGBColor(247,  83, 148)
    ViolinBrown             = base.RGBColor(103,  68,   3)
    Viridian                = base.RGBColor( 64, 130, 109)
    ViridianGreen           = base.RGBColor(  0, 150, 152)
    VistaBlue               = base.RGBColor(124, 158, 217)
    VividAmber              = base.RGBColor(204, 153,   0)
    VividAuburn             = base.RGBColor(146,  39,  36)
    VividBurgundy           = base.RGBColor(159,  29,  53)
    VividCerise             = base.RGBColor(218,  29, 129)
    VividCerulean           = base.RGBColor(  0, 170, 238)
    VividCrimson            = base.RGBColor(204,   0,  51)
    VividGamboge            = base.RGBColor(255, 153,   0)
    VividLimeGreen          = base.RGBColor(166, 214,   8)
    VividMalachite          = base.RGBColor(  0, 204,  51)
    VividMulberry           = base.RGBColor(184,  12, 227)
    VividOrange             = base.RGBColor(255,  95,   0)
    VividOrangePeel         = base.RGBColor(255, 160,   0)
    VividOrchid             = base.RGBColor(204,   0, 255)
    VividRaspberry          = base.RGBColor(255,   0, 108)
    VividRed                = base.RGBColor(247,  13,  26)
    VividRedTangelo         = base.RGBColor(223,  97,  36)
    VividSkyBlue            = base.RGBColor(  0, 204, 255)
    VividTangelo            = base.RGBColor(240, 116,  39)
    VividTangerine          = base.RGBColor(255, 160, 137)
    VividVermilion          = base.RGBColor(229,  96,  36)
    VividViolet             = base.RGBColor(159,   0, 255)
    VividYellow             = base.RGBColor(255, 227,   2)
    Volt                    = base.RGBColor(206, 255,   0)
    
    WageningenGreen  = base.RGBColor(206, 255,   0)
    WarmBlack        = base.RGBColor(  0,  66,  66)
    Watermelon       = base.RGBColor(240,  92, 133)
    WatermelonRed    = base.RGBColor(191,  65,  71)
    Waterspout       = base.RGBColor(164, 244, 249)
    WeldonBlue       = base.RGBColor(124, 152, 171)
    Wenge            = base.RGBColor(100,  84,  82)
    Wheat            = base.RGBColor(245, 222, 179)
    White            = base.RGBColor(255, 255, 255)
    WhiteChocolate   = base.RGBColor(237, 230, 214)
    WhiteCoffee      = base.RGBColor(230, 224, 212)
    WhiteSmoke       = base.RGBColor(245, 245, 245)
    WildBlueYonder   = base.RGBColor(162, 173, 208)
    WildOrchid       = base.RGBColor(212, 112, 162)
    WildStrawberry   = base.RGBColor(255,  67, 164)
    WildWatermelon   = base.RGBColor(252, 108, 133)
    WillpowerOrange  = base.RGBColor(253,  88,   0)
    WindsorTan       = base.RGBColor(167,  85,   2)
    Wine             = base.RGBColor(114,  47,  55)
    WineRed          = base.RGBColor(177,  18,  38)
    WineDregs        = base.RGBColor(103,  49,  71)
    WinterSky        = base.RGBColor(255,   0, 124)
    WinterWizard     = base.RGBColor(160, 230, 255)
    WintergreenDream = base.RGBColor( 86, 136, 125)
    Wisteria         = base.RGBColor(201, 160, 220)
    WoodBrown        = base.RGBColor(193, 154, 107)
    
    Xanadu = base.RGBColor(115, 134, 120)
    
    YaleBlue       = base.RGBColor( 15,  77, 146)
    YankeesBlue    = base.RGBColor( 28,  40,  65)
    Yellow         = base.RGBColor(255, 255,   0)
    CrayolaYellow  = base.RGBColor(252, 232, 131)
    MunsellYellow  = base.RGBColor(239, 204,   0)
    NCSYellow      = base.RGBColor(255, 211,   0)
    PantoneYellow  = base.RGBColor(254, 223,   0)
    ProcessYellow  = base.RGBColor(255, 239,   0)
    RYBYellow      = base.RGBColor(254, 254,  51)
    YellowGreen    = base.RGBColor(154, 205,  50)
    YellowOrange   = base.RGBColor(255, 174,  66)
    YellowRose     = base.RGBColor(255, 240,   0)
    YellowSunshine = base.RGBColor(255, 247,   0)
    
    Zaffre           = base.RGBColor(  0,  20, 168)
    ZinnwalditeBrown = base.RGBColor( 44,  22,   8)
    Zomp             = base.RGBColor( 57, 167, 142)
